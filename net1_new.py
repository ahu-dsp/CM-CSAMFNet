...
class ChannelAttentionModule(nn.Module):
    def __init__(self, channel, ratio=8):
        ...
        # self.avg_conv1=  nn.Conv2d(channel, channel, kernel_size=3, stride=1, padding=1)
        # self.max_conv1 = nn.Conv2d(channel, channel, kernel_size=3, stride=1, padding=1)
        # self.avg_conv2 = nn.Conv2d(channel, channel, kernel_size=3, stride=1, padding=1)
        # self.max_conv2 = nn.Conv2d(channel, channel, kernel_size=3, stride=1, padding=1)
        # self.relu=nn.ReLU()
        # self.conv=nn.Conv2d(2*channel, channel, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, x):
        # Max=self.max_conv2(self.relu(self.max_conv1(self.max_pool(x))))
        # Avg = self.avg_conv2(self.relu(self.avg_conv1(self.avg_pool(x))))
        # F=torch.cat([Max,Avg],1)
        # F=self.conv(F)
        # F=self.sigmoid(F)
        # return F
...
#         self.conv5 = ConvLayer11(in_channels=channel, out_channels=channel, kernel_size=5, stride=1)
#         self.conv3_1 = ConvLayer11(in_channels=channel, out_channels=channel, kernel_size=3, stride=1)
#
#         self.conv3_2 = ConvLayer11(in_channels=channel, out_channels=channel, kernel_size=3, stride=1)
#         self.conv1 = ConvLayer11(in_channels=channel, out_channels=channel, kernel_size=1, stride=1)
#
#
#     def forward(self,x):
